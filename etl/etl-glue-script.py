...
from pyspark.sql.functions import lit, col, when, expr, to_date, date_format, year, dayofmonth, floor, row_number, hour, rand
...
master_dyf = glueContext.create_dynamic_frame.from_options(
    connection_type="s3",
    connection_options={"paths": ["s3://data-lake-gr3/data-2019/part-00000-f3389a53-21c4-4db1-aa98-e96af46b9665-c000.snappy.parquet"]},
    format="parquet"
)
...
# Convert DynamicFrame to DataFrame and take a random sample of 50,000 rows
master_df = master_dyf.toDF().orderBy(rand(seed=42)).limit(50000)
...
sample_dyf = DynamicFrame.fromDF(transformed_df, glueContext, "sample_dyf")

# Write partitioned output to S3 by year
glueContext.write_dynamic_frame.from_options(
    frame=sample_dyf,
    connection_type="s3",
    connection_options={
        "path": "s3://second-glue-bkt-grp-three-nyc/testing-master-data/",
        "partitionKeys": ["year"]
    },
    format="parquet",
